...
To configure a collection of owned types `OwnsMany` should be used in `OnModelCreating`.

As any entity type the owned types need a primary key. If there are no good candidates on the CLR type EF Core will create one, but since these types are defined through a collection the foreign key is not unique, so we can't use just it as the primary key as we do for `OwnsOne`.

The two most straightforward solutions are:
- Using a single non-FK property. The contained values would need to be unique across all owners (e.g. if Parent 1 has Child 1, then Parent 2 cannot have Child 1), so the value doesn't have any inherent meaning. Since the FK is not part of the PK its values can be changed, so you could move a child from one parent to another one, however this usually goes against aggregate semantics.
- Using the FK and an additional `int` property. The additional property value now only needs to be unique for a given parent (so there can be a Parent 1 with Child 1,1 and Parent 2 with Child 2,1). By making the FK part of the PK the relationship between the owner and the owned entity becomes immutable and reflects aggregate semantics better. This is what EF does by default.
...
Notice the `WithOwner` call used to configure the navigation property point back at the owner.
